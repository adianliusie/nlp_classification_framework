...
from ...models.hydra_model import TransformerHydraModel
from ...utils.dir_helper_hydra import DirHelperHydra
...
class HydraTrainer():
    ...
    def __init__(self, exp_name:str, m_args:namedtuple):
        self.dir = DirHelperHydra(exp_name)
    ...
    def set_up_helpers(self, m_args:namedtuple):
        ...
        self.model = TransformerHydraModel(trans_name=m_args.transformer, num_classes=num_classes)
    ...
    def train(self, args:namedtuple):
        ...
        for epoch in range(args.epochs):
            ...
            for k, batch in enumerate(train_b, start=1):
                ...
                self.dir.update_layer_acc_metrics(layer_hits=output.layer_hits, 
            ...
            if args.wandb:
                dev_perf = {f'dev_{k}':v for k, v in vars(perf).items()}
                wandb.log(dev_perf)

            # save performance if best dev performance 
            if perf.loss < best_epoch[1]:
                best_epoch = (epoch, perf.loss, perf.acc_11)
            ...
            if args.wandb:
                test_perf = {f'test_{k}':v for k, v in vars(perf).items()}
                wandb.log(test_perf)
    ...
    def model_output(self, batch):
        ...
        logits_list = output.logits_list
        loss_list = [F.cross_entropy(logit, batch.labels) for logit in logits_list]
        loss = sum(loss_list)/12
        
        #calculate batch accuracy for each layer
        layer_hits = []
        for logit in logits_list:
            hits = torch.argmax(logit, dim=-1) == batch.labels
            hits = torch.sum(hits[batch.labels != -100]).item()
            layer_hits.append(hits)
                             
        num_preds = torch.sum(batch.labels != -100).item()

        return SimpleNamespace(loss=loss, logits_list=logits_list, 
                               layer_hits=layer_hits, num_preds=num_preds)
